...
class DB:
    def __init__(self, path: str = "ofgem.db") -> None:
        ...
        # Log the *absolute* path so we can point sqlite at the same file
        abs_path = os.path.abspath(self.path)
        print("##### USING DB FILE:", abs_path, "#####")
    ...
    def list_org_controls_for_site_view(
            self,
            org_id: int,
            site_id: int | None = None,
    ) -> list[dict]:
        ...
        Semantics:
          - site_id is None → show *corporate-only* controls (no site links)
          - site_id is not None → show controls explicitly linked to that site_id
                                   PLUS corporate controls (no site links)
        """
        with self._conn() as conn, closing(conn.cursor()) as cur:
            if site_id is None:
                # corporate only: controls with no site links
                cur.execute(
                    """
                    SELECT oc.*
                    ...
                    WHERE oc.org_id = ?
                      AND NOT EXISTS (SELECT 1
                                      FROM org_control_sites cs
                                      WHERE cs.org_control_id = oc.id)
                    ORDER BY oc.title
                    """,
                    (int(org_id),),
                )
                ...
                cur.execute(
                    ...
                    SELECT DISTINCT oc.*
                    FROM org_controls oc
                             LEFT JOIN org_control_sites cs
                                       ON cs.org_control_id = oc.id
                    WHERE oc.org_id = ?
                      AND (
                        -- explicitly linked to this site
                        cs.site_id = ?
                            -- or corporate: no links at all
                            OR NOT EXISTS (SELECT 1
                                           FROM org_control_sites cs2
                                           WHERE cs2.org_control_id = oc.id)
                        )
                    ORDER BY oc.title
                    """,
    ...
    def list_sites_for_control(self, org_id: int, org_control_id: int) -> list[dict]:
        """List sites linked to a control (many-to-many)."""
        with self._conn() as conn, closing(conn.cursor()) as cur:
            cur.execute(
                """
                SELECT s.id, s.name, s.code, s.location
                ...
                WHERE cs.org_control_id = ?
                  AND s.org_id = ?
                ORDER BY s.name
                """,
                (int(org_control_id), int(org_id)),
            )
            return [dict(r) for r in cur.fetchall()]

    def set_sites_for_control(self, org_id: int, org_control_id: int, site_ids: list[int]) -> None:
        """
        Replace the set of sites linked to this control.
        Passing an empty list means 'corporate control' (applies to all sites).
        """
        clean_ids = sorted({int(sid) for sid in site_ids if sid})
        with self._conn() as conn, closing(conn.cursor()) as cur:
            cur.execute(
                "DELETE FROM org_control_sites WHERE org_control_id = ?",
                (int(org_control_id),),
            )
            if clean_ids:
                cur.executemany(
                    """
                    INSERT OR IGNORE INTO org_control_sites (org_control_id, site_id)
                    VALUES (?, ?)
                    """,
                    [(int(org_control_id), sid) for sid in clean_ids],
                )
            conn.commit()
